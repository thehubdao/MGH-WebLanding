...
void main() {
    float t = u_time * .4;
    vec3 color = vec3(0.0);
    vec2 uv = vUv - 0.5; // ( gl_FragCoord.xy -.5 * u_resolution.xy ) / u_resolution.y;
    vec2 st = vec2(
        length( uv ) * 1.5,
        atan( uv.y, uv.x )
    );
...
export const referenceFragmentShader = `
varying vec2 vUv;
#define PI 3.14159265359
#define TWO_PI 6.28318530718
#define HALF_PI 1.57079632679
...
void main() {
  ...
  float y = fbm3d(
    ...
    gl_FragColor = vec4(smoothstep( .3, .4, c ), smoothstep( .4, .55, c ), smoothstep( .2, .55, c ),1.0);
